import { add, multiply, normalize, TreeRandom, vec3 } from '../math'
...
export function growWhorledCrown(
  ...
  for (let tier = 0; tier < tierCount; tier += 1) {
    ...
    const tierRadius = (
      profile.tierRadiusFloor + parameters.crownRadius * Math.pow(1 - u, profile.tierRadiusExponent)
    ) *
    ...
    for (let branch = 0; branch < branchCount; branch += 1) {
      ...
      for (let station = 0; station < stationCount; station += 1) {
        const along = 0.32 + station / Math.max(1, stationCount - 1) * 0.68
        ...
        organs.push({
          ...
          radius: profile.organRadius[0] +
            (1 - u) * (profile.organRadius[1] - profile.organRadius[0]),
      ...
      for (let hanging = 0; hanging < hangingCount; hanging += 1) {
        ...
        for (let station = 0; station < profile.hangingStations; station += 1) {
          organs.push({
            ...
            axis: hangingAxis,
            radius: profile.organRadius[0] * 0.9,
            depth: profile.organDepth * 0.9,
